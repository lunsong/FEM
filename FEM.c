...
int bisect(int *arr, int i, int j, int x){
    ...
    printf("bisect not found\n");
    return -1;
}

#define idx(i,j) bisect(\
    indices_int,indptr_int[i##_int], indptr_int[i##_int+1]-1, j##_int)
...
void FEM(
    ...
    *indptr_int = 0;
	...
	sort(indices_int+*indptr_int, indices_int+*N_indices_int);
	*(++indptr_int) = *N_indices_int;
    }
	...
	q[1][1] = p[3][1]-p[0][1];
	q[1][2] = p[3][2]-p[0][2];
	double V6 = inner(q[0],q[1]);
	for(int i=0; i<4; i++){
	    int i_int = to_interior[i];
	    if(i_int==-1) continue;
	    int l = (i+1)%4;
	    int k = (i+2)%4;
	    int m = (i+3)%4;
	    cross(p[l],p[k],q[0],p[m]);
	    data[idx(i,i)] = inner(q[0],q[0]) / (6*V6);
	    for(int j=0; j<4; j++){
		if(i==j) continue;
		int j_int = to_interior[j];
		if(j_int==-1) continue;
		if(j>i){
		    data[idx(i,j)] = data[idx(j,i)];
		    continue;
		}
		int foo = 2*3*4 / (i+1) / (j+1);
		int l = (i+1)%4;
		int k = (i+2)%4;
		if(l==j) l = (i+3)%4;
		else if(k==j) k = (i+3)%4;
		...
		data[idx(i,j)] += - inner(q[0],q[1]) / (6*V6);
	    }
	}
    }
}
